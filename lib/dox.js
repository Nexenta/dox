...
exports.parseComments = function(js, options){
  ...
  // trailing code
  if (buf.trim().length && comments.length) {
    comment = comments[comments.length - 1];
    code = buf.trim();
    comment.code = code;
    comment.ctx = exports.parseCodeContext(code);
...
exports.parseComment = function(str, options) {
  ...
  if (~str.indexOf('\n@')) {
    var tags = '@' + str.split('\n@').slice(1).join('@@@');
    comment.tags = tags.split('@@@').map(exports.parseTag);
    comment.isMan = comment.tags.some(function(tag){
        return 'man' == tag.type;
    });
...
exports.parseTag = function(str) {
  var tag = {} 
    ...
    , type = tag.type = tag.type.replace(/\n*$/, '');
...
exports.parseCodeContext = function(str){
  ...
  if (/^(\w+)\.(staticM|m)ethod\(\'(\w+)\', *function *\((.*)\) *\{$/.exec(str)) {
    return {
        type: RegExp.$2 === 'staticM' ? 'class method' : 'instance method'
      , name: RegExp.$3
      , class: RegExp.$1
      , string: RegExp.$3 + '(' + RegExp.$4 + ')'
    };
  // function statement
